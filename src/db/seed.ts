...
import type { BusynessPattern } from '../types'

// ─── Busyness helpers ─────────────────────────────────────────────────────────

function emptyWeek(): BusynessPattern {
...
function peak(hour: number, peakHour: number, sigma: number, amplitude: number): number {
  return Math.round(
    amplitude * Math.exp(-0.5 * ((hour - peakHour) / sigma) ** 2)
  )
}

function jitter(n: number, range = 6): number {
  return Math.min(100, Math.max(0, Math.round(n + (Math.random() - 0.5) * range)))
}

function makePattern(
  cfg: Array<{ days: number[]; peakHour: number; sigma: number; amplitude: number }>
): BusynessPattern {
...
const BUSYNESS: Record<string, BusynessPattern> = {
  jazz: makePattern([
...
const INTERESTS: Array<{ userId: string; category: string; weight: number; source: string; engagementCount: number }> = [
...
function availabilitySlots(): Array<{ id: string; userId: string; dayOfWeek: number; hour: number }> {
  const rows: Array<{ id: string; userId: string; dayOfWeek: number; hour: number }> = []
  for (const u of USERS) {
    // Everyone available Fri (5) and Sat (6) evenings 18–23
    for (const day of [5, 6]) {
      for (let h = 18; h <= 23; h++) {
        rows.push({ id: uuidv4(), userId: u.id, dayOfWeek: day, hour: h })
      }
    }
...
function run(): void {
  ...
  const insertUser = db.prepare(
    'INSERT OR REPLACE INTO users (id, name, email, lat, lng, city) VALUES (?, ?, ?, ?, ?, ?)'
  )
  ...
  const insertEdge = db.prepare(
    'INSERT OR REPLACE INTO social_edges (user_id, friend_id, strength, initiator_score, mutual_friends) VALUES (?, ?, ?, ?, ?)'
  )
  const insertEngagement = db.prepare(
    'INSERT OR REPLACE INTO venue_engagements (id, user_id, venue_id, level) VALUES (?, ?, ?, ?)'
  )
  const insertSlot = db.prepare(
    'INSERT OR REPLACE INTO availability_slots (id, user_id, day_of_week, hour) VALUES (?, ?, ?, ?)'
  )
  ...
  const seed = db.transaction(() => {
    ...
    for (const i of INTERESTS) {
      insertInterest.run(uuidv4(), i.userId, i.category, i.weight, i.source, i.engagementCount)
    }
    for (const v of VENUES) {
      insertVenue.run(
        v.id, v.name, v.category,
        JSON.stringify(v.tags), v.lat, v.lng, v.address, v.city,
        v.rating, v.priceLevel, JSON.stringify(v.busynessPattern),
        JSON.stringify(v.photos), v.vibeDescription,
        v.qualityScore, v.trendingScore, v.engagementCount,
      )
    }
    ...
    for (const eng of ENGAGEMENTS) {
      insertEngagement.run(uuidv4(), eng.userId, eng.venueId, eng.level)
    }
