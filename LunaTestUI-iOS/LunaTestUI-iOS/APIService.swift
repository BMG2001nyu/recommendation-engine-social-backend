...
class APIService: ObservableObject {
    ...
    func getVenues(city: String? = nil, category: String? = nil, limit: Int = 20) async throws -> VenueListResponse {
        ...
        if let city { query += "&city=\(city.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? city)" }
        if let category { query += "&category=\(category.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? category)" }
    ...
    func expressInterest(userId: String, venueId: String, level: String) async throws -> VenueEngagement {
        try await request("/api/interests", method: "POST", body: ["userId": userId, "venueId": venueId, "level": level])
    }
    ...
    // MARK: - Plans
    func createPlan(venueId: String, createdBy: String, scheduledTime: String, inviteeIds: [String] = []) async throws -> LunaPlan {
        var body: [String: Any] = ["venueId": venueId, "createdBy": createdBy, "scheduledTime": scheduledTime]
    ...
    func sendInvites(planId: String, userIds: [String], invitedBy: String) async throws -> [PlanParticipant] {
        try await request("/api/plans/\(planId)/invites", method: "POST", body: ["userIds": userIds, "invitedBy": invitedBy])
    }

    func respondToInvite(planId: String, userId: String, response: String) async throws -> PlanParticipant {
        try await request("/api/plans/\(planId)/respond", method: "POST", body: ["userId": userId, "response": response])
    }
    ...
    // MARK: - Events
    func postEvent(type: String, userId: String, venueId: String? = nil) async throws {
